 ...
 * @author yuki
 */
public class NewAdvancedSearchResultActivity extends AdActivity implements
		IWebPostRunnable<NewAdvancedSearchResult[]>, OnItemClickListener, OnClickListener, IAdvancedSearch {
	...
	private static class RetainData {
		NewSearchLoader loader;
		RouteData data;
		NewAdvancedSearchResult result;
		int selectedIndex = -1;
	}
	...
	public void onClick(View v) {
		if (v.getId() == R.id.change_search_condition) {
			navigateUpActivity();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (callingActivity != null) {
			...
			String canonicalName = AdvancedSearchConditionActivity.class.getCanonicalName();
			isDirectBack = className.equals(canonicalName);
		}
		...
		if (retainData == null) {
			...
			if (retainData.result != null) {
				onPostRunnable(new NewAdvancedSearchResult[] { retainData.result });
			}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		NewBusSearchList list = (NewBusSearchList) parent.getAdapter();
		BusTransferSearch item = list.getItem(position);

		Intent intent = new Intent(this, NewAdvancedDetailActivity.class);
		intent.putExtra(IntentValues.TRANSITION_ADVANCED_NEW_SEARCH, item);
		startActivity(intent);
	...
	public void onNewIntent(Intent intent) {
		...
		if (intent.hasExtra(IntentValues.TRANSITION_ADVANCED_SEARCH)) {
			...
			retainData.loader = new NewSearchLoader(this, true, this);
			retainData.loader.execute(new RouteData[] { retainData.data });
		}
	}
	 ...
	 */
	@Override
	public void onPostRunnable(final NewAdvancedSearchResult[] results) {
		NewAdvancedSearchResult result = results[0];
	...
	protected void onResume() {
		...
		retainData.data.setEnableTransfer(true);
	...
	private void setResultAdapter(NewAdvancedSearchResult result) {
